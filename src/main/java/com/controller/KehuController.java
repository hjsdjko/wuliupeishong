...
public class KehuController {
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        params.put("kehuDeleteStart",1);params.put("kehuDeleteEnd",1);
    ...
    */
    ...
    public R save(@RequestBody KehuEntity kehu, HttpServletRequest request){
        ...
        Wrapper<KehuEntity> queryWrapper = new EntityWrapper<KehuEntity>()
            .eq("kehu_uuid_number", kehu.getKehuUuidNumber())
            .eq("kehu_gongshi", kehu.getKehuGongshi())
...
//            return R.error(511,"永远不会进入");
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids){
        ...
        ArrayList<KehuEntity> list = new ArrayList<>();
        for(Integer id:ids){
            KehuEntity kehuEntity = new KehuEntity();
            kehuEntity.setId(id);
            kehuEntity.setKehuDelete(2);
            list.add(kehuEntity);
        }
        if(list != null && list.size() >0){
            kehuService.updateBatchById(list);
        }
        return R.ok();
    }
...
//                            kehuEntity.setKehuGongshi(data.get(0));                    //公司名称 要改的
//                            kehuEntity.setKehuAddress(data.get(0));                    //公司地址 要改的
//                            kehuEntity.setSexTypes(Integer.valueOf(data.get(0)));   //性别 要改的
//                            kehuEntity.setKehuName(data.get(0));                    //联系人 要改的
//                            kehuEntity.setKehuPhone(data.get(0));                    //联系方式 要改的
//                            kehuEntity.setKehuEmail(data.get(0));                    //邮箱 要改的
//                            kehuEntity.setKehuDelete(1);//逻辑删除字段
//                            kehuEntity.setCreateTime(date);//时间
                        ...
                        //查询是否重复
                         //客户编号
                        List<KehuEntity> kehuEntities_kehuUuidNumber = kehuService.selectList(new EntityWrapper<KehuEntity>().in("kehu_uuid_number", seachFields.get("kehuUuidNumber")).eq("kehu_delete", 1));
                        if(kehuEntities_kehuUuidNumber.size() >0 ){
                         ...
                         //联系方式
                        List<KehuEntity> kehuEntities_kehuPhone = kehuService.selectList(new EntityWrapper<KehuEntity>().in("kehu_phone", seachFields.get("kehuPhone")).eq("kehu_delete", 1));
